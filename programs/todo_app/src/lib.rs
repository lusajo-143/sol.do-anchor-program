...
pub mod todo_app {
    ...
    pub fn create_todo(ctx: Context<CreateTodo>, description: String, due_date: String) -> Result<()> {
        let todo_list = &mut ctx.accounts.todo_list;
    ...
    pub fn change_todo_status(ctx: Context<ChangeTodoStatus>, task_id: u64) -> Result<()> {

        let todo_list = &mut ctx.accounts.todo_list;
        let todo_list = &mut ctx.accounts.todo_list;

        if let Some(task_index) = todo_list.todo_tasks.clone().iter().position(|task| task.id == task_id) {
            todo_list.todo_tasks[task_index].is_completed = !todo_list.todo_tasks[task_index].is_completed;
        } else {
...
pub struct Initialize<'info> {
    #[account(
        init,
        payer = payer,
        space = 8 + 32 + 1 + 4 + (50 * (8 + 1 + (4 + 100) + 4 + 15)),
