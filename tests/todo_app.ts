...
describe("todo_app", () => {
    ...
    it("Create todo 01!", async () => {
        // Add your test here.
        const tx = await program.methods.createTodo("Task 01", "02 Jan, 2025")
            .accounts({
                todoList: pda,
                payer: anchor.AnchorProvider.env().wallet.publicKey
            })
    ...
    it("Create todo 02!", async () => {
        // Add your test here.
        const tx = await program.methods.createTodo("Task 02", "03 Jan, 2025")
            .accounts({
                todoList: pda,
                payer: anchor.AnchorProvider.env().wallet.publicKey
            })
    ...
    it("Complete todo!", async () => {
        // Add your test here.
        const tx = await program.methods.changeTodoStatus(new BN(taskId))
            .accounts({
                todoList: pda,
                payer: anchor.AnchorProvider.env().wallet.publicKey
            })
    ...
    // it("Fail Complete todo!", async () => {
    //     // Add your test here.
    //     const tx = await program.methods.changeTodoStatus(new BN(5), true)
    //         .accounts({
    //             todoList: pda,
    //             payer: anchor.AnchorProvider.env().wallet.publicKey
    //         })
